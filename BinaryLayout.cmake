...
function(binary_enable_stack_size target size)
  ...
  message(STATUS "BinaryLayout [${target}]: stack size = ${size}")
endfunction()
...
function(binary_enable_heap_size target size)
  ...
  message(STATUS "BinaryLayout [${target}]: heap size = ${size}")
endfunction()
...
function(binary_enable_cfg target)
  if(MSVC)
    ...
    message(STATUS "BinaryLayout [${target}]: Control Flow Guard enabled")
  else()
...
function(binary_enable_ltcg target)
  if(MSVC)
    ...
    message(STATUS "BinaryLayout [${target}]: LTCG enabled")
  elseif(CLANG)
    _bl_add_compile(${target} -flto=thin)
    _bl_add_link(${target} -flto=thin)
    message(STATUS "BinaryLayout [${target}]: ThinLTO enabled")
  elseif(GCC)
    _bl_add_compile(${target} -flto)
    _bl_add_link(${target} -flto)
    message(STATUS "BinaryLayout [${target}]: LTO enabled")
  else()
