...
function(_pgo_apply_generate target)
  ...
  if(MSVC)
    ...
    pp_scope("PGO" "${target}" "MSVC instrumented build (GENPROFILE)")
  elseif(CMAKE_CXX_COMPILER_ID MATCHES "Clang")
    ...
    pp_scope("PGO" "${target}" "Clang instrumented build (profile-instr-generate)")
  elseif(CMAKE_CXX_COMPILER_ID STREQUAL "GNU")
    _pgo_add_compile(${target} -fprofile-generate=${PGO_PROFILE_DIR})
    _pgo_add_link(${target} -fprofile-generate=${PGO_PROFILE_DIR})
    pp_scope("PGO" "${target}" "GCC instrumented build (profile-generate)")
  else()
...
function(_pgo_apply_use target)
  if(MSVC)
    ...
    pp_scope("PGO" "${target}" "MSVC optimized build (USEPROFILE + LTCG)")
  elseif(CMAKE_CXX_COMPILER_ID MATCHES "Clang")
    ...
    pp_scope("PGO" "${target}" "Clang optimized build (profile-use + ThinLTO)")
  elseif(CMAKE_CXX_COMPILER_ID STREQUAL "GNU")
    ...
    pp_scope("PGO" "${target}" "GCC optimized build (profile-use + LTO)")
  else()
    message(WARNING "PGO: Unsupported compiler for PGO")
  endif()
endfunction()
...
function(pgo_apply_compile_global)
  if(NOT GIN_PGO_PHASE STREQUAL "OFF")
    if(MSVC)
      ...
      pp_scope("PGO" "GLOBAL compile" "/GL enabled for all translation units")
    endif()
  endif()
endfunction()
...
function(pgo_apply target)
  if(GIN_PGO_PHASE STREQUAL "generate")
    if(MSVC)
      ...
      pp_scope("PGO" "${target}" "MSVC linker GENPROFILE")
    elseif(CMAKE_CXX_COMPILER_ID MATCHES "Clang")
  ...
  elseif(GIN_PGO_PHASE STREQUAL "use")
    if(MSVC)
      ...
      pp_scope("PGO" "${target}" "MSVC linker USEPROFILE + LTCG")
    elseif(CMAKE_CXX_COMPILER_ID MATCHES "Clang")
